...
---
Asynchronous programming is recommended pattern to scale performance of applications due to its non-blocking I/O characteristic ([read more](https://docs.microsoft.com/en-us/dotnet/standard/async-in-depth#deeper-dive-into-tasks-for-an-io-bound-operation){:target="_blank"} :link: ). In this article, we'll compare execution of asynchronous & synchronous request processing in an ASP.NET MVC application to understand its benefits.
